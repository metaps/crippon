<template>
  <div id="app">
      ...
      <app-menu></app-menu>
      Wallet:<input v-model="walletAddress"  size="50"></input>
      <button v-on:click="getJsonFromGivenWallet">get</button><br>
      My Wallet:{{this.msg}}<button v-on:click="getJsonFromMyWallet">get my assets</button>
...
export default {
  ...
  mounted: async function(){
    console.log("mounted")
    await web3.eth.getAccounts().then(function(value){myAddress=value[0]})
    this.msg = myAddress
  ...
  methods: {
    getJsonFromGivenWallet: async function () {
      let num = 0
      await contract.methods.balanceOf(this.walletAddress).call().then(function(value){
      ...
      let tokenids = []
      for(let i=0;i<num;i++){
        await contract.methods.tokenOfOwnerByIndex(this.walletAddress,i).call().then(function(value){
    ...
    getJsonFromMyWallet: async function () {
      this.msg = myAddress
      let num = 0
      await contract.methods.balanceOf(this.msg).call().then(function(value){
        num = parseInt(value._hex,16)
      })
      let tokenids = []
      for(let i=0;i<num;i++){
        await contract.methods.tokenOfOwnerByIndex(this.msg,i).call().then(function(value){
          tokenids.push(parseInt(value._hex,16))
        })
      }
